...
https://www.hivemq.com/[HiveMQ MQTT Client] library. It supports both **MQTT 5**
(the default) and **MQTT 3.1.1**, selected with the `mqttVersion` option.
...
By default, the HiveMQ component connects using MQTT 5. Set `mqttVersion=MQTT_3_1_1`
to connect to a broker using MQTT 3.1.1 instead:
